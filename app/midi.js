/*
 * MIDIJS credit: @surikov
 * https://github.com/surikov/webaudiofont/blob/master/examples/MIDIFile.js
 */
var UTF8 = {
    ...
    getCharCode: function (bytes, byteOffset, charLength) {
        ...
        // Test UTF8 integrity
        mask =
            "00000000".slice(0, charLength) +
            1 +
            "00000000".slice(charLength + 1);
        ...
        mask =
            "0000".slice(0, charLength + 1) + "11111111".slice(charLength + 1);
        charCode +=
            (bytes[byteOffset] & parseInt(mask, 2)) << (--charLength * 6);
    ...
    getBytesForCharCode: function (charCode) {
        ...
        throw new Error(
            "CharCode " + charCode + " cannot be encoded with UTF8."
        );
    },
    setBytesFromCharCode: function (charCode, bytes, byteOffset, neededBytes) {
        ...
        if (1 == neededBytes) {
            ...
            bytes[byteOffset++] =
                (parseInt("1111".slice(0, neededBytes), 2) <<
                    (8 - neededBytes)) +
...
MIDIEvents.createParser = function midiEventsCreateParser(
    ...
    if (stream instanceof DataView) {
        stream = {
            ...
            pos: function () {
                return (
                    "0x" + (this.buffer.byteOffset + this.position).toString(16)
                );
            },
            end: function () {
                    ...
                    switch (event.subtype) {
                        case MIDIEvents.EVENT_META_SEQUENCE_NUMBER:
                            if (strictMode && 2 !== event.length) {
                                throw new Error(
                                    stream.pos() + " Bad metaevent length."
                                );
                            }
...
MIDIEvents.writeToTrack = function midiEventsWriteToTrack(
    ...
    for (i = 0, j = events.length; i < j; i++) {
        ...
        if (events[i].type === MIDIEvents.EVENT_MIDI) {
            ...
            if (
                -1 !== MIDIEvents.MIDI_2PARAMS_EVENTS.indexOf(events[i].subtype)
            ) {
            ...
            if (events[i].length >>> 21) {
                destination[index++] =
                    ((events[i].length >>> 21) & 0x7f) | 0x80;
            }
            if (events[i].length >>> 14) {
                destination[index++] =
                    ((events[i].length >>> 14) & 0x7f) | 0x80;
            }
    ...
    if (!buffer) {
        ...
        if (
            !(
                0xff ===
                    this.datas.getUint8(
                        MIDIFileTrack.HDR_LENGTH + (trackLength - 3)
                    ) &&
                0x2f ===
                    this.datas.getUint8(
                        MIDIFileTrack.HDR_LENGTH + (trackLength - 2)
                    ) &&
                0x00 ===
                    this.datas.getUint8(
                        MIDIFileTrack.HDR_LENGTH + (trackLength - 1)
                    )
            )
        ) {
            throw new Error(
                "Invalid MIDIFileTrack (0x" +
                    ...
                    " (" +
                    (MIDIFileTrack.HDR_LENGTH + (trackLength - 1)).toString(
                        16
                    ) +
                    ")."
            );
...
export class MIDIFile {
    ...
    closeNote = function (event, song) {
        ...
        for (var i = 0; i < track.notes.length; i++) {
            if (
                ...
                track.notes[i].duration =
                    event.playTime / 1000 - track.notes[i].when;
                break;
            }
        }
    };
    addSlide = function (event, song) {
        var track = this.takeTrack(event.channel, song);
        for (var i = 0; i < track.notes.length; i++) {
            if (
                track.notes[i].duration == 0.0000001 && //
    ...
    startDrum = function (event, song) {
        var beat = this.takeBeat(event.param1, song);
        beat.notes.push({
            when: event.playTime / 1000,
        });
    };
    takeTrack = function (n, song) {
                    ...
                    if (
                        ...
                        if (
                            ...
                            if (events[i].param1 == 7) {
                                if (events[i].channel != 9) {
                                    var track = this.takeTrack(
                                        events[i].channel,
                                        song
                                    );
                                    track.volume =
                                        events[i].param2 / 127 || 0.000001;
                                }
                            }
                        } else {
                            if (
    ...
    getEvents = function (type, subtype) {
        var events;
        var event;
        var playTime = 0;
        var filteredEvents = [];
            ...
            for (i = 0, j = this.tracks.length; i < j; i++) {
                ...
                while (event) {
                    playTime += event.delta
                        ? (event.delta * tickResolution) / 1000
                        : 0;
                    ...
                    if (
                        ...
                        (!subtype ||
                            (event.subtype && event.subtype === subtype))
                    ) {
    ...
    setTrackEvents = function (index, events) {
        var bufferLength;
        var destination;
...
export function startPlay(song, isStopped) {
    currentSongTime = 0;
    songStart = audioContext.currentTime;
    nextStepTime = audioContext.currentTime;
    var stepDuration = 44 / 1000;
    tick(song, stepDuration, isStopped);
}

function tick(song, stepDuration, isStopped) {
    if (audioContext.currentTime > nextStepTime - stepDuration) {
        ...
        // loop
        if (currentSongTime > song.duration || isStopped()) {
            return;
        }
    }
...
export function startLoad(song, callback) {
    ...
    for (var i = 0; i < song.tracks.length; i++) {
        // distortion guitar
        const guitarId = 321;
        var info = player.loader.instrumentInfo(guitarId);
        song.tracks[i].info = info;
        song.tracks[i].id = guitarId;
